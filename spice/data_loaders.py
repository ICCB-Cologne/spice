...
def _observed_table_override(stem):
    """A cohort-supplied path for one of the observed tables, from `input_files.<stem>`.

    The packaged observed tables are keyed by ASSEMBLY, so one hg19 file is shared by every hg19
    cohort -- but their values are where a COHORT's own events stop either side of the centromere and
    at the telomeres, so they are cohort-specific in substance. This override lets each cohort supply
    its own pair (generated by pipeline-peak-detection/src/spice/make_observed_tables.py --out-dir).

    It is now REQUIRED, not an override: the packaged `*_observed` tables were removed on 2026-09-07
    because they were never reference data. See `_require_observed` below for why absence is an error
    ...
    if not isinstance(cfg, dict):
        return None
    return _resolve_optional_input_file((cfg.get('input_files') or {}).get(stem))


def _require_observed(stem):
    """Resolve a cohort's observed table, or fail loudly. There is no packaged fallback.

    The `*_observed` tables record where a COHORT's own inferred events stop either side of the
    centromere and at the telomeres. That is a property of the DATA, not of the reference assembly,
    so a file keyed by assembly cannot be correct for two cohorts on the same build -- measured on
    chr1's `large` scale, the span excluded from the loci search was 25.2 Mb in the packaged hg19
    file against 7.8 Mb for TCGA's own table, 7.2 for CNSistent and 0.4 for the synthetic cohorts.
    The packaged file matched no cohort at all; it was one earlier dataset's measurement, shipped as
    if it were a coordinate table beside chrom_lengths and centromeres, which ARE assembly-only.

    Until 2026-08-27 every hg19 cohort silently shared it, and giving each its own moved loci counts
    by -7% (CNSistent) to +39% (neutral), because these tables set the searchable sequence that
    `N_loci_spacing` seeds candidates along. A silent fallback therefore does not degrade a run, it
    invalidates it -- so this raises instead.
    """
    path = _observed_table_override(stem)
    if path is None:
        raise FileNotFoundError(
            f"{stem} is not configured. It is COHORT-SPECIFIC data, not a packaged coordinate "
            f"table, so there is no default to fall back on: a shared per-assembly copy would "
            f"silently detect this cohort against another dataset's centromere definition. "
            f"Generate the pair with pipeline-peak-detection/src/spice/make_observed_tables.py "
            f"--final-events <cohort final_events.tsv> --out-dir <dir>, then set "
            f"`input_files.{stem}` to it (the pipeline's INJECT_ASSEMBLY_LOCI and PLOTTING steps do "
...
def load_centromeres(extended=True, observed=False, pad=None):
    ...
    if observed:
        path = _require_observed(stem)          # cohort data: no packaged fallback
        log_debug(logger, f'Loading {stem} from the cohort table {path}')
        centromeres = pd.read_csv(path, sep='\t', header=[0, 1], index_col=0)
    else:
...
def load_telomeres_observed():
    '''Create file using create_observed_centromeres_and_telomeres'''
    path = _require_observed('telomeres_observed')   # cohort data: no packaged fallback
    log_debug(logger, f'Loading telomeres_observed from the cohort table {path}')
